...
<%
	...
	if(idx == null || idx.trim().equals("")){
		response.sendRedirect("BoardList.board"); //cpage=1 , ps=5
		return;
	}
	BoardService service = BoardService.getInBoardService();
	Board board = service.board_EditContent(idx);
	if(board == null){
		//만약 받환받은 Board가 없다면(수정요청한 게시글이 없다면)
		out.print("데이터 오류");
		//다시 목록으로 이동시켜줄 링크 추가
		out.print("<hr><a href='BoardList.board'>목록가기</a>");
		return;
	}
...
<body>
	...
	<div id="pageContainer">
		<div style="padding-top: 25px; text-align: center">
			<!-- form 시작 -->
			<form name="edit" action="BoardEditOk.board" method="POST">
				<center>
					<table width="90%" border="1">
						<tr>
						<!--보여주고 수정 할것은 text, 
							안보여주고 서버로 가져가야 할 정보는 hidden,
							보여주고 서버로 안가져갈거면 enable, 
							보여주고 수정안되고 서버로 가져갈거면 readonly -->
