...
public class BoardDao {
	...
	public Board getContent(int idx) {
		...
		try {
			conn = ds.getConnection();
			String sql="select * from jspboard where idx=?"; //* 하지 말자!! (가독성 떨어진다)
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, idx);
	...
	public int deleteOk(String idx , String pwd) {
		...
		case 2: (네이버)원본글만 삭제 -> 나머지 처리 (텍스트 형태 (원본글 삭제 표시) (step, depth)
				step, depth로 비교 (같은 refer 중 step, defer가 0인 애가 없으묜!!)
				상태값만 넣어주는 것 (삭제된 표시만 해두고, 보여지지 않도록 한 뒤 실제로는 지우지 않음) 
				> 해당 상태값을 확인해서 답글처리
		case 3: 삭제시 삭제되었을 표시 ( 게시판 설계 (delok :삭제여부 컬럼 :1) >> 삭제 : 0 >> update .. 0
		case 4: 삭제 못하게 한다 (답글이 있으면) refer count > 1
		
		//여기서는 그냥 답글까지 다 지우도록 설계해두었당~~~~
		*/
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
				...
				//댓글 먼저 다 삭제해야 게시글 삭제 가능
				//댓글이 게시글의 글번호를 참조하고 있기 떄문에!!
				...
				if(rs.next()) { //삭제글은 존재
					//사용자가 입력한 비번 , DB 비번
					 if(pwd.equals(rs.getString("pwd"))) {
						 ...
						 //두개를 하나의 논리적 단위
						 //JDBC : auto commit 
						 //## 댓글만 삭제되고 게시글이 삭제되지 않을 수 있으므로 반드시 트랜잭션 처리 ##
